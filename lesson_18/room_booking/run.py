...
def seed_database():
    with app.app_context():
        if User.query.first():
            print("Baza juz zawiera dane. Pomijam seeding.")
            return

        print("Tworzenie przykladowych danych...")
        ...
        equipment_list = [
            ...
            Equipment(name="Naglośnienie", icon="volume-up"),
        ]
        db.session.add_all(equipment_list)

        rooms = [
            Room(name="Sala A1", capacity=10, floor=1, description="Mala sala do spotkan", hourly_rate=50),
            Room(name="Sala B2", capacity=20, floor=2, description="Srednia sala z projektorem", hourly_rate=80),
            Room(name="Sala Konferencyjna", capacity=50, floor=3, description="Duza sala na prezentacje", hourly_rate=150),
            Room(name="Pokoj Kreatywny", capacity=8, floor=1, description="Sala do burzy mozgow", hourly_rate=60),
        ]
        ...
        users = [
            ...
            User(name="Piotr Wisniewski", email="piotr@firma.pl", department="Marketing"),
            User(name="Maria Dabrowska", email="maria@firma.pl", department="IT", is_admin=True),
        ]
        db.session.add_all(users)
        db.session.commit()

        titles = ["Spotkanie zespolu", "Code review", "Prezentacja projektu",
        ...
        db.session.commit()
        print("Baza danych wypelniona przykladowymi danymi.")
