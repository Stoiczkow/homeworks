...
async def check_status(host):
    ...
    await client.close()
    return f"Strona {host} odpowiedział statusem {result.status}"


async def main():
    tasks = asyncio.gather(
        check_status("https://wp.pl"),
        check_status("https://onet.pl"),
        check_status("https://gazeta.pl"),
        check_status("https://tvp.pl"),
        check_status("https://krakow.pl")
    )
