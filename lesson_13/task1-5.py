...
# Szukanie po inpucie usera

title = input("Podaj tytuł książki: ")

cursor.execute("""
                SELECT * FROM ksiazki WHERE tytul = ?                
                """, (title,))

found_books = cursor.fetchall()

if found_books:
    print(f"Znalezione książki o tym tytule: {found_books}")
else:
    print(f"Nie znaleziono książek o tytule {title}")

# Dodawanie rekordów po inpucie usera
